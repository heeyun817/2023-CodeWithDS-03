...
class WalkChatRoom(models.Model):  # 채팅방
    board = models.OneToOneField(WalkBoard, on_delete=models.CASCADE)
    # 어디 글에 해당되는 채팅방인지
    user_group = models.ManyToManyField(User)
...
class WalkChatMessage(models.Model):
    ...
    message = models.TextField()
    timestamp = models.DateTimeField(auto_now_add=True)
    chatRoom = models.ForeignKey(WalkChatRoom, on_delete=models.CASCADE)
